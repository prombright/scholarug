...
onMounted(async () => {
  try {
    ...
    bootError.value = e.response?.data?.message || 'Could not load the HR portal.'
  } finally {
...
// app_hr.php injects (avoids a flash of the wrong theme); this just keeps
// the button's icon and localStorage in sync with it from here on.
