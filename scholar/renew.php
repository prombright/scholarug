...
$stmt = $pdo->prepare('SELECT * FROM subscriptions WHERE school_id = ?');
...
$plans = Plans::forApp('scholar');

$pendingStmt = $pdo->prepare(
    "SELECT sc.reference, sc.network, sc.phone, sc.amount, sc.currency, sc.created_at
     FROM subscription_charges sc JOIN subscriptions s ON s.id = sc.subscription_id
     WHERE s.school_id = ? AND sc.status = 'pending' ORDER BY sc.created_at DESC"
);
$pendingStmt->execute([$schoolId]);
...
<title>ScholarUg | Renew Subscription</title>
...
<p>Keep your school's Scholar account active.</p>
...
<div class="stat-grid">
    <div class="stat">
        ...
        <div class="value"><?= htmlspecialchars(ucfirst($subscription['status']), ENT_QUOTES, 'UTF-8') ?></div>
    </div>
    <div class="stat">
        <div class="label"><?= $subscription['status'] === 'trialing' ? 'Trial Ends' : 'Current Period Ends' ?></div>
        <div class="value">
            <?php
            $deadline = $subscription['status'] === 'trialing' ? $subscription['trial_ends_at'] : $subscription['current_period_end'];
...
<a class="back" href="<?= htmlspecialchars(SCHOLAR_BASE, ENT_QUOTES, 'UTF-8') ?>/dashboard.php">← Back to Dashboard</a>
</div>
...
(function () {
    ...
    function pollStatus(reference, onDone) {
        ...
        pollTimer = setInterval(function () {
            ...
            fetch("subscription_status.php?reference=" + encodeURIComponent(reference), { credentials: "same-origin" })
    ...
    form.addEventListener("submit", function (event) {
        ...
        fetch("subscription_initiate.php", { method: "POST", body: formData, credentials: "same-origin" })
            .then(function (res) { return res.json(); })
            .then(function (data) {
                if (data.status === "pending") {
                    ...
                    pollStatus(data.reference, function (outcome, error) {
                        if (outcome === "successful") {
                            waitingText.textContent = "Payment received! Updating your subscription...";
                            ...
                            waitingText.textContent = "Still waiting for confirmation -- your subscription will update automatically once approved.";
                        }
                    });
