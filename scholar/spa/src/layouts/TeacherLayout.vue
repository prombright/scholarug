...
// The page carries a <base href="assets/spa-teacher/"> (see app_teacher.php)
// so its own JS/CSS resolve correctly -- but that means every OTHER
// relative link on the page would resolve against that same folder unless
// built from the absolute SCHOLAR_BASE app_teacher.php injects instead.
const SB = window.__SCHOLAR_BASE__ || '/ScholarUg/scholar/'

// `route: '...'` items are migrated pages living inside this same SPA --
// rendered as <router-link>, no reload. Everything else is still the
// classic PHP app and gets a plain <a href>, full reload, same as always.
const NAV_ITEMS = [
  { key: 'dashboard', label: 'Dashboard', icon: 'bi-grid-1x2', route: '/' },
  { key: 'marks', label: 'Marks Entry', icon: 'bi-pencil-square', route: '/marks' },
  { key: 'analytics', label: 'Performance Analytics', icon: 'bi-bar-chart-line', route: '/analytics' },
  { key: 'timetable', label: 'My Timetable', icon: 'bi-calendar3', route: '/timetable' },
  { key: 'attendance', label: 'Roll Call', icon: 'bi-calendar-check', route: '/attendance' },
    ...
    <aside class="sidebar" :class="{ open: mobileOpen }">
      ...
      <nav class="sidebar-nav">
        <template v-for="item in NAV_ITEMS" :key="item.key">
          <router-link v-if="item.route" :to="item.route" :class="{ active: route.path === item.route }">
