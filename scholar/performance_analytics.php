...
if ($sel_assignment) {
    $rows = analytics_fetch_scores($pdo, $school_id, $sel_class, $sel_subject, $current_term, $current_year);
    ...
    $ct_stmt->execute([$sel_class, $school_id]);
    $is_class_teacher_here = (int) $ct_stmt->fetchColumn() === $staff_id;

    if ($is_class_teacher_here) {
        $allRows = analytics_fetch_scores($pdo, $school_id, $sel_class, null, $current_term, $current_year);

        // Per-subject class averages, to surface the class's strongest/
        // weakest subject -- summarize once per subject using only that
        // subject's rows.
        $bySubject = [];
        ...
        // Overall per-student standing: the mean of each student's own
        // per-subject averages -- fair across students who take different
        // numbers of assessments in different subjects, same principle a
        // report card's overall aggregate already uses.
        $byStudentSubjectAvg = [];
